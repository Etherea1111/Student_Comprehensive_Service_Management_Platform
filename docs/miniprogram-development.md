...
当前 `data/mockData.js` 中内置了演示用户，角色为“班团骨干”，具备公开内容维护和操作日志查看权限。配置后端后，登录页会默认使用微信登录，并支持 10 位学号密码登录、首次微信绑定、修改密码和找回密码；角色和权限由服务端返回。
...
- 后端服务：负责微信登录、权限校验、文件上传、知识库维护、流程配置、日志记录。
